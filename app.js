(() => {
  ...
  function renderPaychecks() {
    ...
    tbody.innerHTML = checks.length ? checks.map(p => {
      ...
      return `<tr>
        <td data-label="Date"><button class="btn btn-small btn-secondary" data-select-paycheck="${p.id}">${new Date(p.date+"T00:00:00").toLocaleDateString()}</button></td>
        <td data-label="Base">${money(p.amount)}</td><td data-label="Extra">${money(p.extra)}</td><td data-label="Total">${money(paycheckTotal(p))}</td>
        <td data-label="Assigned Bills">${money(assigned)}</td><td data-label="Remaining">${money(paycheckRemaining(p))}</td>
  ...
  function renderBills() {
    ...
    tbody.innerHTML = bills.length ? bills.map(b => {
      ...
      return `<tr>
        <td data-label="Status">${statusBadge(billStatus(b))}</td>
        <td data-label="Bill"><strong>${escapeHtml(b.name)}</strong><br><small class="muted">${escapeHtml(b.category || "Uncategorized")}${b.recurring ? " • Recurring" : ""}</small></td>
        <td data-label="Amount">${money(b.amount)}</td>
        <td data-label="Due">${new Date(b.dueDate+"T00:00:00").toLocaleDateString()}</td>
        <td data-label="Priority">${b.priority}</td>
        <td data-label="Assigned Check">${check ? new Date(check.date+"T00:00:00").toLocaleDateString() : "Unassigned"}</td>
        <td data-label="Paid"><input type="checkbox" data-toggle-paid="${b.id}" ${b.paid ? "checked" : ""} /></td>
        <td class="row-actions" data-label="Actions">
  ...
  document.addEventListener("click", (e) => {
    ...
    if (nav) {
      ...
      $("pageTitle").textContent = nav.dataset.title || nav.textContent.trim();
      const moreMenu = $("moreMenu");
      if (moreMenu?.open) moreMenu.close();
      document.querySelectorAll(".nav-btn").forEach(b => {
        if (b.dataset.page === nav.dataset.page) b.classList.add("active");
      });
    }
  ...
  $("moreNavBtn")?.addEventListener("click", () => $("moreMenu").showModal());
  $("closeMoreMenu")?.addEventListener("click", () => $("moreMenu").close());
